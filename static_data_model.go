...
func (s *StaticDataModel) SetDistribution(number_of_symbols uint, probability []float64) {
	...
	for k := uint(0); k < s.data_symbols; k++ {
		...
		s.distribution[k] = uint(sum * (1 << DM__LengthShift))
		sum += p
		if s.table_size == 0 {
			continue
		}
